---
name: feature-plan
description: 在功能设计与问题诊断阶段理解用户真实需求、收敛方案，并按需输出面向用户与 AI 的文本草案
---
...
定位：本 skill 只负责理解、诊断、规划、取舍和文本草案整理。需要时可以产出或更新 Markdown 规划文档，但不进入代码、配置、脚本、测试、资源或数据实施。
...
- 约束声称：
- 已有证据：
当前裁决：继续理解 | 输出方案 | 输出 bug 排查路径 | 输出执行前提草案
下一步：
```

执行要点：

1. 先给当前理解，再做题目校准，不要一上来只追问或只讲实现。
...
7. 需要文档承载时，尽早给出 Markdown 草案；fp 只到文档与方案，不进入非文档实施。
8. 命中附件、低信息输入、步进式提问或候选方案扩展时，读 `feature-plan/references/planning-playbook.md`；命中 bug、输出策略或交付边界时，读 `feature-plan/references/delivery-playbook.md`。

## 理解、校准与扩展

1. 先理解用户真实需求、真实想法、权衡偏好和不可接受结果，再定义问题、取舍方案和组织文档。
2. 默认把输入拆成六层：真正目标、成功标准、表层诉求、候选手段、约束声称、已有证据；不要把它们混写成同一层。
...
6. 输出可以是用户说明、AI 任务草案、临时方案、bug 排查路径或规划 Markdown；选择哪种，取决于当前最能帮助用户拍板和继续推进的承载形式。
...
9. fp 不负责代码、配置、脚本、测试、资源或数据实施；若用户要落地实现，只负责把边界、输入、风险与验收标准整理成可交接草案。
