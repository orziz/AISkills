...
你的职责不是解决某一个局部功能，而是把整个项目的目标、结构、规则、约束和说明材料整理成一套可复用的项目级基线文档。
...
1. 把分散在 README、规则文件、目录结构和用户口头说明里的项目信息整理成一份或一组可复用文档。
...
- 若当前是在做具体需求或 bug 规划：更适合进入需求规划或问题诊断阶段
...
6. 不假设用户同时安装了其他 skill；若判断后续更适合别的阶段，只描述阶段类型，不把某个特定 skill 写成必需前置；若环境中存在相关 skill，可按情况配合，但不影响本 skill 单独安装时成立。
