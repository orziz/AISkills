...
`skill-sync` 现在通过仓库内脚本 `odai/scripts/skill-sync.js` 执行。
...
2. 对命中统一术语校验的 skill，先检查术语基线、并行 support files 与 README 关键分节 / 锚点是否存在，并扫描禁用旧口径；校验不过就报错并停止。
3. 读取 `工程根目录/skills/<skill-name>/SKILL.md`，将其作为主正文来源。
4. 若同目录下存在 `odai/references/`、`odai/assets/` 或 `odai/scripts/`，则将这些目录视为当前 skill 的附属资源，一并同步到各手动安装目标目录。
5. Claude 侧 wrapper 元数据也以 unified source 为准；若需要稳定写入 `allowed-tools`、`argument-hint` 等字段，就在源文件 frontmatter 中显式提供当前脚本约定的字段，而不是从现有 `.claude/` 产物反推或保留。
6. 在当前仓库里，默认只处理 `odai`；若 frontmatter 提供 `replaces`，同步清理旧 skill 名对应的安装产物与 README 条目。
7. 生成目标文件时，默认采用“固定模板 + 正文直拷”策略：不要改写源文件正文，不要重组，不要风格化重写。
8. 若正文中引用了 `skills/<skill-name>/SKILL.md`、`skills/<skill-name>/references/`、`skills/<skill-name>/assets/`、`skills/<skill-name>/scripts/`，或 `odai/references/`、`odai/assets/`、`odai/scripts/` 下的相对路径，生成手动安装文件时都要改写成目标目录内真实可用的入口与资源路径。
9. 先解析源文件 frontmatter：读取 `name`、`description`，并按需读取 `readme-section`、`replaces` 与 Claude 侧前端元数据字段。
10. 再取源文件 frontmatter 之后的正文，作为唯一正文载荷；正文段落顺序、标题层级、代码块、措辞保持原样。
11. 直接基于源文件内容生成或覆盖以下目标文件：
   - `.claude/commands/<skill-name>.md`
   - `.github/skills/<skill-name>/SKILL.md`
   - `.trae/skills/<skill-name>.md`
   - `.trae/rules/<skill-name>.md`
12. 若存在附属资源目录，则在以下位置同步同名目录：
   - `.claude/commands/<skill-name>/`
   - `.github/skills/<skill-name>/`
   - `.trae/skills/<skill-name>/`
   - `.trae/rules/<skill-name>/`
13. 更新 `README.md` 前，先确认 `### 面向大多数使用者`、`### 2. 手动安装`、`### 维护流程`、`## 目录说明` 等关键分节仍存在，且标准安装入口与目录说明锚点未丢；缺一项就停止，不把 README 当成可失败的软目标。
14. 更新 `README.md` 时，只处理“当前已提供”列表：
