...
6. 涉及环境能力判断、多 agent 多模型合议、内部收束、冻结方案后的独立复查或分歧升级时，统一读取 `references/dao/parallel-consensus-playbook.md` 并按该文件执行；若总控 agent 在本轮里已通过直查或自动桥接回填确认可分配模型列表或可用模型标识，且主流程需要为席位分配模型，则继续读取 `references/dao/model-selection-baseline.md`。
