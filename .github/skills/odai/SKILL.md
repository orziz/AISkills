...
9. 涉及并行增强时分两档执行：仅当运行环境被明确识别为 Copilot，且满足 `references/dao/parallel-consensus-playbook.md` 的完整能力条件时，才允许在内部启用“多 agent 多模型合议”；若不满足完整多模型条件，但满足子 agent 调用、统一 skill 下发与合法阶段的部分能力条件，则允许退而启用“子 agent 辅助复核模式”。两档都必须显式承接同一份 `odai` skill 约束，至少拿到同版 canonical 入口、当前命中模块与必要 support files 的规则，不得让外部通用 agent 脱离本 skill 自行发挥；是否可启用、如何触发、怎样收束与何时升级统一按该手册执行。若当前环境连统一 skill 约束都无法可靠传递给子 agent，则视为能力不足，回到各模块原流程；不得把辅助复核包装成多模型参与。
...
6. 涉及环境能力判断、多 agent 多模型合议、内部收束、冻结方案后的独立复查或分歧升级时，统一读取 `references/dao/parallel-consensus-playbook.md` 并按该文件执行；若当前环境还暴露了可分配模型列表，且主流程需要为席位分配模型，则继续读取 `references/dao/model-selection-baseline.md`。
7. 只要内部拉起子 agent 或切换子模型做合议，就必须把同一份 `odai` skill 契约显式传给它：至少要求读取 canonical 入口、当前命中模块与必要 support files；默认按 `assets/dao/subagent-execution-template.md` 组织统一下发包。模型本身不视为自动携带 skill，不能因为换了模型就默认继承规则。
