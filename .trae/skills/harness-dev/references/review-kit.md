...
3. 主动扩展相邻模块、边界条件、失败路径、隐藏约束、上下游依赖、协作影响与维护成本，不要只围绕用户原话做单线程判断。
4. 所有主动扩展出来的内容，只能记为候选解释、假设项、风险项或待确认项。
5. 若主动扩展触及路线、边界、验收、授权、风险等级或正式裁决，就必须问用户或补足证据，不能直接拍板。
...
8. 若环境支持结构化提问组件，必须优先使用；若环境确实不支持，再退回文本确认。
...
- 默认把用户输入视为待校验线索，主动扩展相邻场景、边界条件、反例和隐藏约束
...
- 文档至少覆盖：当前理解、目标与非目标、目标用户与使用场景、页面或模块范围、信息架构与内容层级、关键交互流程、页面状态与边界情况、视觉与布局方向、文案与反馈规则、响应式与兼容性要求、待确认项、验收口径
