...
- 但这些都只是方法来源，不是运行时依赖。
- 即使用户没有安装 `project-guide`、`feature-plan`、`design-spec`、`review-sslb`、`implement-code`，本 skill 也必须完整执行。
- 不允许要求用户“先装另外几个 skill 再来用 harness”。

如果当前项目或当前环境本身已经具备 `project-guide`、`feature-plan`、`design-spec`、`review-sslb`、`implement-code`，而且环境明确支持直接调用、显式切换或等价的 stage handoff，那么 harness 可以在局部阶段主动借力；但这是一种优化，不是前置条件。

### 按需借力原则

不要为了看起来完整，就把每个任务都强行走成：

`project-guide -> feature-plan -> design-spec -> review-sslb -> implement-code`
...
- 用户口中的 `ds`、`design spec`、`设计说明那个`、`整理页面和交互那个`，默认都优先匹配到 `design-spec`
...
- 当前主要矛盾是页面、流程、状态、交互或设计说明缺口，而不是代码细节时，可借力 `design-spec`
- 当前主要矛盾是正式审查、diff 审查、合并前把关或历史文件排查时，可借力 `review-sslb`
...
3. 若环境明确支持真实调用，且当前阶段明显更适合 `project-guide`、`feature-plan`、`design-spec`、`review-sslb` 或 `implement-code`，默认优先真实调用，不要偷懒只在内部模拟。
...
- 需要先把页面、流程、状态、交互或设计说明收敛清楚时，优先真实借力 `design-spec`
- 需要正式审查、diff 审查、目录级排查、完整三省六部输出时，优先真实借力 `review-sslb`
...
若用户明确要求“按 sslb 过堂”“用 r-sslb 看一下”“按 review-sslb 审”，或虽然没说名字、但语义上明显是在要“正式三省六部审查 / 完整过堂 / 严格审查”那套能力，且环境支持真实调用，应优先真实借力 `review-sslb`，不要只在 harness 内部给一个看起来像 sslb 的摘要版。
...
6. 若环境支持真实借力且当前阶段语义上明显命中 `project-guide`、`feature-plan`、`design-spec`、`review-sslb` 或 `implement-code` 对应能力，是否已经优先真实调用；若未调用，是否写明原因
