...
这里不再新增第二个对外 skill。你的职责是把能力收进 `odai` 这一套统一入口下面，形成可维护的 canonical module source：`references/modules/<module-name>.md` 与必要的 namespaced support files。
...
1. canonical 路径默认是 `references/modules/<module-name>.md`；第一版就应是真实源文件，而不是只在聊天里给草稿。
...
2. 创建或更新 `references/modules/<module-name>.md`。
